...
BEGIN
  ...
  FOR r IN
    SELECT p.email, COALESCE(NULLIF(p.first_name, ''), 'Beloved') AS first_name
    FROM public.students s
    JOIN public.profiles p ON p.id = s.profile_id
    WHERE s.cohort_id = v_active_cohort_id
      AND s.admission_status ILIKE 'pending'
      AND p.email IS NOT NULL
      AND p.email <> ''
      AND p.email LIKE '%_@_%.__%'
  LOOP
    BEGIN
...
<div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; color: #1f2937;">
  ...
  <div style="background: #f9fafb; padding: 16px 24px; text-align: center; border-top: 1px solid #e5e7eb;">
    <p style="font-size: 12px; color: #9ca3af; margin: 0;">© Spirit Life School of Ministry · spiritlifesom.org</p>
      ...
      v_sent := v_sent + 1;
      RAISE NOTICE 'Queued welcome email to % (%)', r.email, r.first_name;
    EXCEPTION WHEN OTHERS THEN
      v_skipped := v_skipped + 1;
      RAISE WARNING 'Failed to queue email for %: %', r.email, SQLERRM;
    END;
  END LOOP;

  RAISE NOTICE 'Welcome email backfill complete. Sent: %, Skipped: %', v_sent, v_skipped;
END $$;